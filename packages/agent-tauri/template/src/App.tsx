  ...
  return (
    ...
    >
      <div onClick={(e) => e.stopPropagation()} style={{
        ...
        <h2 style={{ margin: "0 0 4px", fontSize: 20 }}>My Agent</h2>
        <p style={{ margin: "0 0 16px", color: "#666", fontSize: 13 }}>Version {version}</p>
        <p style={{ margin: "0 0 8px", fontSize: 13, color: "#555" }}>
          Built with{" "}
          <a href="https://github.com/earendil-works/pi" target="_blank" rel="noopener noreferrer" style={{ color: "#1976d2" }}>
            @earendil-works/agent-base
          </a>
        </p>
        <button onClick={onClose} style={{
          padding: "8px 24px", borderRadius: 6, border: "none",
          background: "#1976d2", color: "#fff", fontSize: 13, cursor: "pointer",
        }}>Close</button>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "100vh", padding: 32, fontFamily: "system-ui, -apple-system, sans-serif" }}>
      <div style={{ maxWidth: 480, width: "100%" }}>
        ...
        <p style={{ marginBottom: 24, color: "#666", fontSize: 14 }}>Choose your LLM provider and enter an API key to get started.</p>
        ...
        <button onClick={handleSave} disabled={saving} style={{ width: "100%", padding: "10px 0", borderRadius: 6, border: "none", background: saving ? "#90caf9" : "#1976d2", color: "#fff", fontSize: 14, fontWeight: 600, cursor: saving ? "default" : "pointer" }}>
    ...
    <div ref={barRef} style={{ display: "flex", alignItems: "center", height: 32, background: "#f5f5f5", borderBottom: "1px solid #ddd", userSelect: "none", fontFamily: "system-ui, -apple-system, sans-serif", fontSize: 13, position: "relative", flexShrink: 0 }}>
      {menus.map((menu, mi) => (
        <div key={mi} style={{ position: "relative" }}>
          <div onClick={() => setOpenMenu(openMenu === mi ? null : mi)} onMouseEnter={() => { if (openMenu !== null) setOpenMenu(mi); }} style={{ padding: "4px 10px", cursor: "pointer", borderRadius: 4, background: openMenu === mi ? "#e0e0e0" : "transparent", marginLeft: 2 }}>
            {menu.label}
          </div>
          {openMenu === mi && (
...
function ChatPage({ onConfigure }: { onConfigure: () => void }) {
  ...
  const send = useCallback(async () => {
    ...
    try {
      ...
      setMessages((prev) => { const copy = [...prev]; copy[copy.length - 1] = { role: "assistant", text: errMsg }; return copy; });
